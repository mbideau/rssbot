...
def msg_fix_from_header(msg): # pylint: disable=too-many-statements
    ...
    if 'From' in msg:
        ...
        if not email_detected:
            logging.debug("no email detected in msg From '%s'", from_str)
            from_bot = get_config().get('message', 'from')
        ...
        # ensure the From header contains the non-email part in quotes
            ...
            if not from_str.startswith('"') and not from_str.endswith('"'):
                logging.debug("email miss quotes in msg From '%s'", from_str)
                from_bot = get_config().get('message', 'from')
    ...
    if from_str_ascii != from_str:
        logging.debug("the 'From' header is not in us-ascii format")
        logging.debug("replacing msg From '%s' by '%s'",
